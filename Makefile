...
.PHONY: all debug release install uninstall container-build print-tags

all: debug release

debug:
	mkdir -p build/debug
	go build -o build/debug/$(SERVICE) ./cmd/$(SERVICE)

release:
	mkdir -p build/release
	go build -ldflags "-s -w" -o build/release/$(SERVICE) ./cmd/$(SERVICE)

install:
	go install -ldflags "-s -w" github.com/swayrider/swctl/cmd/swctl
